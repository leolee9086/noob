const Ajv = requireInstall("ajv")
const ajv = new Ajv() // options can be passed, e.g. {allErrors: true}
const express = requireInstall('express')
const fg = requireInstall('fast-glob')
const apiLevel = [
    "admin",
    "write",
    "read"
...
export default class Api {
    ...
    route(path,apiRouter){
        if(apiRouter instanceof APIRouter){
            apiRouter.parent = this
            this.app.use(path,apiRouter.router)
            this.subRouters.push({path,apiRouter})
        }
        else {
            console.error(`需要传入一个APIRouter实例,但传入了${router}`)
        }
    }
}
export class APIRouter{
    ...
    regist(path, describe) {
        if (typeof path == 'string') {
            describeApi(path, describe, this.router, this.doc)
        }
        else if (path instanceof Array) {
            path.forEach(
                p => {
                    describeApi(p, describe, this.router, this.doc)
                }
            )
        }
    }
    route(path,apiRouter){
        ...
        if(apiRouter instanceof APIRouter){
            apiRouter.parent = this
            this.router.use(path,apiRouter.router)
            this.subRouters.push({path,apiRouter})
        }
    }
}