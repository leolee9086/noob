...
const Mime = requireInstall('Mime');
...
export default  function addDevSurppoert(app) {
    app.use('/deps', async (req, res) => {
        let filePath = (naive.public.config.backend.filesys.workspaceDir + `/conf/naiveConf/deps/esm/${req.url}`).replace(/\?/g, '.')
        let fixPath = (naive.public.config.backend.filesys.workspaceDir + `/conf/naiveConf/deps/esmFix/${req.url}`).replace(/\?/g, '.')
        ...
        if (fs.existsSync(filePath)) {
            ...
            console.warn(`模块${req.url}重定向到naive设置文件夹/deps/esm`)
            ...
            /*let source = await */fetch('https://esm.sh' + req.url+'?bundle', {
                ...
                content=>{
                   // content = "/*" + mime + "*/" + "\n" + content
                    ...
                    res.end(content)
                    try{
                        naive.backend.fileSys.pathConstructor.mkfilep(filePath, content)
                    }catch(e){
                        console.error(e)
                    }
