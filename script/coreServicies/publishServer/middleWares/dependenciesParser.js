...
import {mkfilep} from "../../../util/file.js"   
...
export default  function addDevSurppoert(app) {
    app.use('/deps', async (req, res) => {
        let filePath = (workspaceDir + `/conf/naiveConf/deps/esm/${req.url}`).replace(/\?/g, '.')
        let fixPath = (workspaceDir + `/conf/naiveConf/deps/esmFix/${req.url}`).replace(/\?/g, '.')
