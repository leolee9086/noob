...
<input type="checkbox" class="b3-switch fn__flex-center"${nodeElement.getAttribute("custom-heading-mode") === "1" ? " checked" : ""}></div>`,
        ...
        window.siyuan.menus.menu.append(new MenuItem({
            type: "readonly",
            label: `<div style="margin-left: -18px;white-space: nowrap;" data-type="${type}">${updateHTML}${window.siyuan.languages.createdAt} ${dayjs(id.substr(0, 14)).format("YYYY-MM-DD HH:mm:ss")}</div>`,
        }).element);
        return window.siyuan.menus.menu;
    }
