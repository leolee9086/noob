...
async function 注入块属性(req, res, 渲染结果) {
  let blockid =
    req.params.blockid ||
    req.query.blockid ||
    req.query.id ||
...
async function 生成发布文档头(req, res, 渲染结果) {
...
async function 生成发布文档内容(req, res, 渲染结果) {
...
async function 生成文档标题(req, res, 渲染结果) {
...
async function 修改嵌入块(req, res, 渲染结果) {
  let docId = 渲染结果.block.docInfor.id
  渲染结果 = await 刷新嵌入块(渲染结果, docId)
  return 渲染结果
}
async function 刷新嵌入块(元素, docid) {
...
async function 获取嵌入块内容(嵌入块, docid) {
