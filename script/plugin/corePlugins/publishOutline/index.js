export class publishOutline extends naive.plugin {
  ...
  pipe = [this.生成文档大纲];
  async 生成文档大纲(req, res, 渲染结果) {
    console.log(this);
    let 大纲 = 渲染结果.querySelector('#panelLeft [data-type="outline"]');
    console.log(大纲);
    console.log(渲染结果.block);
    let 大纲内容 = await this.核心api.getDocOutline(
      { id: 渲染结果.block.docInfor.ial.id },
      ""
    );
    console.log(大纲内容);
    let html = this.渲染大纲(大纲内容, 渲染结果);
    大纲.innerHTML = html;
    this.替换大纲条目内容(大纲内容,渲染结果)
  ...
  替换大纲条目内容(大纲内容,渲染结果){
    ...
    let 元素 = 渲染结果.querySelector(
      ...
      if (大纲条目.blocks) {
        this.替换大纲条目内容(大纲条目.blocks,渲染结果)    
    }
    })
  }
  渲染大纲条目内容(大纲内容, 渲染结果) {
