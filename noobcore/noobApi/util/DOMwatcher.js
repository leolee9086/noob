export class DOM监听器 {
    ...
    开始监听(){
        ...
        if(this.监听目标序列&&this.监听目标序列[0]){
                let 监听器 = new MutationObserver((mutationsList, observer)=>{
                    if(Array.from(this.监听目标序列).indexOf(mutationsList[0].target)>=0){
                    this.监听器回调(mutationsList, observer)
                    }

                })
                监听器.observe(document,this.监听选项)            
        }
    }
    ...
    自动刷新监听目标() {
        ...
        //监听整个文档的变化,然后刷新监听器序列
        监听器.observe(document, {attributes: true, childList: true, subtree: true})
    }
    是否DOM序列(判定目标){
